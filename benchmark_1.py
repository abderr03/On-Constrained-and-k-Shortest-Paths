...
ALGO = {
    1: 'Dijkstra (lazy priority queue)',
    #2: 'Dijkstra parallel',
    3: "Bellman-Ford (naive)",
    4: 'Bellman-Ford (optimized)',
    5: '$\Delta$-stepping (sequential)'
}
...
df = pd.DataFrame(columns=[
    'algorithm', 'source', 'target', 'time', 'preprocess_time',
    'computation_time', 'dist', 'num_vertices'
])
...
def generate_chart(dataframe):
    ...
    plt.legend(title='Algorithms',
               ...
               labels=[
                   ALGO[1], '_hidden', ALGO[3], '_hidden', ALGO[4], '_hidden',
                   ALGO[5] + ', with $\Delta = {delta}$'.format(delta=DELTA)
               ])
    plt.xlabel('Path weight', fontweight='bold')
    plt.ylabel('Computation time (in ' + TIME_UNIT_NS + ')', fontweight='bold')
    plt.tight_layout()
    plt.savefig('benchmark_plot_computation_task_1.pdf',
                bbox_inches='tight',
                pad_inches=0.0,
                dpi=DPI)
    ...
    plt.tight_layout()
    plt.savefig('benchmark_plot_total_task_1.pdf',
                bbox_inches='tight',
                pad_inches=0.0,
                dpi=DPI)
    ...
    plt.xlabel('Path weight', fontweight='bold')
    plt.ylabel('Preprocessing time (in ' + TIME_UNIT_NS + ')',
               fontweight='bold')
    plt.tight_layout()
    plt.savefig('benchmark_plot_preprocess_task_1.pdf',
                bbox_inches='tight',
                pad_inches=0.0,
                dpi=DPI)
...
def generate_heatmap_total(dataframe):
    for i in ALGO:
        ...
        plt.title('Heatmap of the total running time (in ' + TIME_UNIT_μS +
                  ') of ' + ALGO[i],
                  fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig('benchmark_total_task_1_' + str(i) + '.pdf',
                    bbox_inches='tight',
                    pad_inches=0.0,
                    dpi=DPI)
...
def generate_heatmap_preprocess(dataframe):
    for i in ALGO:
        ...
        #df1 = df1.astype({'time': 'float'})
        df1['preprocess_time'] = df1[
            'preprocess_time'] / (1000 * 100)  # Convert ns to µs
        plt.figure(figsize=(15, 7))
        heatmap = sns.heatmap(data=df1.pivot(index='source',
                                             columns='target',
        ...
        heatmap.collections[0].colorbar.ax.set_yticklabels(
            list(
                map(lambda x: x + TIME_UNIT_μS, [
                    '$\leqslant$ 0.1', '1', '10','$\geqslant$ 100'
                ])))
        heatmap.set_facecolor("#808080")
        plt.title('Heatmap of the preprocessing time (in ' + TIME_UNIT_μS +
                  ') of ' + ALGO[i],
                  fontweight='bold')
        plt.xticks(rotation=45)
        plt.yticks(rotation=0)
        ...
        plt.ylabel('Source vertex', fontweight='bold')
        plt.tight_layout()
        plt.savefig('benchmark_preprocess_task_1_' + str(i) + '.pdf',
                    bbox_inches='tight',
                    pad_inches=0.0,
                    dpi=DPI)
...
def generate_heatmap_computation(dataframe):
    for i in ALGO:
        ...
        #df1 = df1.astype({'time': 'float'})
        df1['computation_time'] = df1[
            'computation_time'] / 1000  # Convert ns to µs
        plt.figure(figsize=(15, 7))
        heatmap = sns.heatmap(data=df1.pivot(index='source',
                                             columns='target',
        ...
        heatmap.set_facecolor("#808080")
        plt.title('Heatmap of the computation time (in ' + TIME_UNIT_μS +
                  ') of ' + ALGO[i],
                  fontweight='bold')
        plt.xticks(rotation=45)
        plt.yticks(rotation=0)
        ...
        plt.ylabel('Source vertex', fontweight='bold')
        plt.tight_layout()
        plt.savefig('benchmark_computation_task_1_' + str(i) + '.pdf',
                    bbox_inches='tight',
                    pad_inches=0.0,
                    dpi=DPI)
...
def main():
            ...
            for i in vertices:
                for j in vertices:
                    if i != j:
                        ...
                        list_to_write = [1, i, j, NUM_REPEAT, algo_num]
                        if algo_num == 5:
                            list_to_write.append(DELTA)
    ...
    df.to_csv('benchmark_task_1.csv', index=False)


#main()
df1 = pd.read_csv('benchmark_task_1.csv')
generate_chart(df1)
